...
@interface APPosRefundTest : APPosBaseTest
        ...
        case SPIStatusPairedConnected:
            ...
            switch (state.flow) {
                ...
                case SPIFlowIdle:
                    ...
                    if (!self.didProcessCommandDict[@(APCommandRefund)]) {
                        self.didProcessCommandDict[@(APCommandRefund)] = @YES;
                         // 91 will trigger a signature required test in the emulator
                        [self.spi initiateRefundTx:[SPIRequestIdHelper idForString:@"rfnd"] amountCents:100 completion:^(SPIInitiateTxResult *result) {
