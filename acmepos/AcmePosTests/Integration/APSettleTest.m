...
@interface APSettleTest : APPosBaseTest
        ...
        case SPIStatusUnpaired:
            ...
            switch (state.flow) {
                ...
                case SPIFlowPairing: {
                    ...
                    if (pairingState.isAwaitingCheckFromPos && !pairingState.isAwaitingCheckFromEftpos) {
        ...
        case SPIStatusPairedConnected:
            ...
            switch (state.flow) {
                ...
                case SPIFlowIdle:
                    ...
                    if (!self.didProcessCommandDict[@(APCommandSettle)]) {
                        self.didProcessCommandDict[@(APCommandSettle)] = @YES;
                        [self.spi initiateSettleTx:[SPIRequestIdHelper idForString:@"settle"] completion:^(SPIInitiateTxResult *result) {
        ...
        if (state.txFlowState.successState == SPIMessageSuccessStateUnknown) {
            ...
            switch (state.txFlowState.type) {
                ...
                case SPITransactionTypePurchase: {
                    SPIPurchaseResponse *purchaseResponse = nil;

                    if (state.txFlowState.response != nil) {
                        purchaseResponse = [[SPIPurchaseResponse alloc] initWithMessage:state.txFlowState.response];
                ...
                case SPITransactionTypeSettle:
                    ...
                    if (state.txFlowState.response != nil) {
                        ...
                        XCTAssertEqual(state.txFlowState.successState, SPIMessageSuccessStateSuccess);
                        XCTAssertNil(state.txFlowState.response.error);
