//
//  APPosPurchaseWithSignatureTest.m
//  AcmePosTests
//
...
@interface APPosPurchaseWithSignatureTest : APPosBaseTest
            ...
            switch (state.flow) {
                ...
                case SPIFlowIdle:
                    ...
                    if (!self.didProcessCommandDict[@(APCommandPurchaseInitiate)]) {
                        ...
                        [self.spi initiatePurchaseTx:[SPIRequestIdHelper idForString:@"prchs"] amountCents:91 completion:^(SPIInitiateTxResult *result) {
                ...
                case SPIFlowTransaction:
                    ...
                    if (state.txFlowState.isAwaitingSignatureCheck) {
                        ...
                        if (!self.didProcessCommandDict[@(APCommandTxSignAccept)]) {
                            SPILog(@"APCommandTxSignAccept");
                            self.didProcessCommandDict[@(APCommandTxSignAccept)] = @YES;
                            [self.spi acceptSignature:YES];
                        }
                    }
